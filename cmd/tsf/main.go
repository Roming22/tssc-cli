...
func main() {
	...
	// Create TSF-specific application context (metadata/configuration).
	appCtx := api.NewAppContext(
		"tsf",
		api.WithVersion(version),
		api.WithCommitID(commitID),
		api.WithShortDescription("Trusted Software Factory CLI"),
	)

	// TSF-specific MCP server image based on build-time commit ID.
	mcpImage := "quay.io/redhat-user-workloads/rhtap-shared-team-tenant/tsf-cli"
	if commitID == "" {
